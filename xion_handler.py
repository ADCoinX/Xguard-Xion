...
def get_all_balances(balances_json: Dict[str, Any]):
    ...
    for coin in (balances_json.get("balances") or []):
        denom = coin.get("denom"); amount = str(coin.get("amount", "0"))
        if denom == "uxion":
...
async def _probe_endpoint(client: httpx.AsyncClient, base: str, address: str):
    if _cb_blocked(base):
        return base, None, "circuit_open"
    try:
        # account (optional liveness check)
        _ = await _fetch_first_ok(client, base, _account_paths(address))

        balances = await _fetch_first_ok(client, base, _balance_paths(address))
        blist = _parse_balances_shape(balances)
        if not blist:
            _cb_trip(base); return base, None, f"{base} returned empty balances"
        ...
        status = "ok" if tx_count is not None else "partial"; tx_count = tx_count or 0
        ...
        _cb_trip(base); return base, None, f"{base} error: {e}"

async def get_wallet_info(address: str) -> dict:
    if not validate_wallet_address(address):
        return {"address": address, "status": "invalid_address",
    ...
    return {"address": address, "status": "unreachable",
